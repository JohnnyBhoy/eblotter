...
export default function ProvinceManagement() {
  ...
  useEffect(() => {
    api.get('/geography/provinces').then(r => setProvinces(r.data)).finally(() => setLoading(false));
  }, []);
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    try {
      const res = await api.post('/geography/provinces', { psgcCode });
      setProvinces(p => [...p, res.data]);
      setShowForm(false);
      setPsgcCode('');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to register province');
    } finally {
      setSubmitting(false);
    }
  }
