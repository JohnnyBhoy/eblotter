...
export default function AuditLog() {
  ...
  function load(p = 1, q = search) {
    ...
    // Use a dedicated audit log endpoint if available; fallback to blotter stats
            ...
            <tbody className="divide-y divide-gray-100">
              {logs.length === 0 ? (
                ...
                <tr key={log._id || idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 text-gray-500 whitespace-nowrap text-xs">{formatDateTime(log.timestamp || log.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
