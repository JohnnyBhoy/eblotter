...
export default function BarangayManagement() {
  ...
  const [form, setForm] = useState({ psgcCode: '', municipalityDbId: '', provinceDbId: '', punongBarangay: '', barangaySecretary: '', contactNumber: '' });
  const [error, setError] = useState('');
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    Promise.all([
      api.get('/geography/barangays').then(r => r.data),
      api.get('/geography/provinces').then(r => r.data),
    ]).then(([brgys, provs]) => {
      setBarangays(brgys);
      setProvinces(provs);
  ...
  useEffect(() => {
    if (form.provinceDbId) {
      api.get('/geography/municipalities', { params: { provinceId: form.provinceDbId } }).then(r => setMunicipalities(r.data));
    }
  }, [form.provinceDbId]);
