...
export default function ExportBar({ blotterId, summaryParams }: ExportBarProps) {
  function downloadWithAuth(path: string): void {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <button onClick={() => downloadWithAuth(`/export/summary/pdf${qs}`)}
        className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition">
        📄 Export Summary PDF
      </button>
      <button onClick={() => downloadWithAuth(`/export/summary/excel${qs}`)}
        className="flex items-center gap-1.5 px-3 py-1.5 bg-green-700 text-white text-sm rounded-lg hover:bg-green-800 transition">
