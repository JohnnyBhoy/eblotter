...
export default function PSGCSelector({ value = {}, onChange, levels = ['region', 'province', 'municipality', 'barangay'] }) {
    ...
    <div className="space-y-3">
      ...
      {levels.includes('province') && (
        <div>
          ...
          <select className={selectClass} value={province} onChange={handleProvince} disabled={levels.includes('region') && !region}>
            <option value="">Select Province</option>
            {getProvinces(region).map(p => (
      ...
      {levels.includes('barangay') && (
        <div>
          ...
          <select className={selectClass} value={barangay} onChange={e => setBarangay(e.target.value)} disabled={!municipality}>
            <option value="">Select Barangay</option>
            {getBarangays(municipality).map(b => (
