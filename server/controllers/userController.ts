...
export const createUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { username, password, role, fullName, email, contactNumber, barangayId, municipalityId, provinceId } =
      req.body as Record<string, string>;

    const existing = await User.findOne({ username });
    if (existing) { res.status(400).json({ message: 'Username already taken' }); return; }
    ...
    if (role === 'barangay' && barangayId) {
      ...
      if (!barangayDoc) { res.status(400).json({ message: 'Barangay not found' }); return; }
      municipalityDoc = barangayDoc.municipality as Record<string, unknown>;
      provinceDoc = barangayDoc.province as Record<string, unknown>;
      scopeLabel = `${barangayDoc.name}, ${(municipalityDoc?.['name'] as string) || ''}, ${(provinceDoc?.['name'] as string) || ''}`;
    } else if (role === 'municipal' && municipalityId) {
      municipalityDoc = await Municipality.findById(municipalityId).populate('province') as unknown as Record<string, unknown>;
      if (!municipalityDoc) { res.status(400).json({ message: 'Municipality not found' }); return; }
      provinceDoc = municipalityDoc['province'] as Record<string, unknown>;
      scopeLabel = `${(municipalityDoc['name'] as string) || ''}, ${(provinceDoc?.['name'] as string) || ''}`;
    } else if (role === 'provincial' && provinceId) {
      provinceDoc = await Province.findById(provinceId);
      if (!provinceDoc) { res.status(400).json({ message: 'Province not found' }); return; }
      scopeLabel = provinceDoc.name;
    }

    const user = await User.create({
      username, password, role, fullName, email, contactNumber,
      barangay: barangayDoc ? (barangayDoc as unknown as Record<string, unknown>)['_id'] : undefined,
      municipality: municipalityDoc ? (municipalityDoc as Record<string, unknown>)['_id'] : undefined,
      province: provinceDoc ? (provinceDoc as unknown as Record<string, unknown>)['_id'] : undefined,
      scopeLabel,
...
export const updateUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { fullName, email, contactNumber, password } = req.body as Record<string, string>;
