...
export function generateBlotterPDF(blotter, res) {
    ...
    doc.font('Helvetica').text(`${formatDate(blotter.incident?.dateOccurred)}${blotter.incident?.timeOccurred ? ' at ' + blotter.incident.timeOccurred : ''}`);
    ...
    respondents.forEach((r, idx) => {
        ...
        doc.text(`Sex: ${rd['sex'] || 'N/A'}   Age: ${rd['age'] || 'N/A'}`);
        doc.text(`Address: ${formatAddress(rd['address'])}`);
        doc.text(`Contact: ${rd['contactNumber'] || 'N/A'}`);
        doc.text(`Relationship: ${rd['relationshipToComplainant'] || 'N/A'}`);
        doc.moveDown(0.3);
    });
    ...
    if (witnesses.length > 0) {
        ...
        witnesses.forEach((w, idx) => {
            ...
            doc.font('Helvetica').text(`Name: ${wd['firstName'] || ''} ${wd['lastName'] || ''}`);
            doc.text(`Contact: ${wd['contactNumber'] || 'N/A'}`);
            doc.text(`Address: ${wd['address'] || 'N/A'}`);
            if (wd['statement'])
                doc.text(`Statement: ${wd['statement']}`);
            doc.moveDown(0.3);
        });
    }
