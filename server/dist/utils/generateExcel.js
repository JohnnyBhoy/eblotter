...
export async function generateSummaryExcel(blotters, scope, dateRange, res, provincialMode = false) {
    ...
    if (dateRange)
        summaryWs.getCell('A3').value = dateRange;
    summaryWs.addRow([]);
    ...
    if (provincialMode) {
        ...
        const incidentTypes = [...new Set(blotters.map(b => b.incident?.type).filter((t) => Boolean(t)))];
        ...
        blotters.forEach(b => {
            const mun = b.municipality;
            const munName = mun?.['name'] || 'Unknown';
            if (!munData[munName])
                munData[munName] = { total: 0 };
...
//# sourceMappingURL=generateExcel.js.map