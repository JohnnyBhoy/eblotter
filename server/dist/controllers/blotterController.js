...
export const scopeFilter = async (user) => {
    switch (user.role) {
        case 'barangay': {
            const brgyId = user.barangay?._id ?? user.barangay;
        ...
        case 'provincial': {
            const provId = user.province?._id ?? user.province;
...
export const createBlotter = async (req, res, next) => {
    try {
        const user = req.user;
        if (!user.barangay) {
            res.status(400).json({ message: 'User has no assigned barangay' });
            return;
        }
        ...
        const blotter = await Blotter.create({
            ...
            createdBy: user._id,
            recordedBy: {
                name: body['recordedBy']?.['name'] || user.fullName,
                position: body['recordedBy']?.['position'] || 'Barangay Official',
            }
        });
...
export const updateBlotter = async (req, res, next) => {
    try {
        ...
        if (!blotter) {
            res.status(404).json({ message: 'Blotter not found or access denied' });
            return;
        }
...
export const updateStatus = async (req, res, next) => {
    try {
        ...
        const blotter = await Blotter.findOneAndUpdate(filter, { status, lastUpdated: new Date(), updatedBy: req.user._id }, { new: true }).populate('barangay').populate('municipality').populate('province');
        if (!blotter) {
            res.status(404).json({ message: 'Blotter not found' });
            return;
        }
...
export const deleteBlotter = async (req, res, next) => {
    try {
        const blotter = await Blotter.findOneAndUpdate({ _id: req.params['id'], isDeleted: false }, { isDeleted: true, deletedAt: new Date(), deletedBy: req.user._id }, { new: true });
        if (!blotter) {
            res.status(404).json({ message: 'Blotter not found' });
            return;
        }
...
//# sourceMappingURL=blotterController.js.map