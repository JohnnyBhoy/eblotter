...
export const getRegions = (_req, res) => { res.json(psgc.regions); };
...
export const registerProvince = async (req, res, next) => {
    try {
        ...
        if (!psgcProvince) {
            res.status(400).json({ message: 'Province not found in PSGC data' });
            return;
        }
...
export const registerMunicipality = async (req, res, next) => {
    try {
        ...
        if (!psgcMun) {
            res.status(400).json({ message: 'Municipality not found in PSGC data' });
            return;
        }
...
export const registerBarangay = async (req, res, next) => {
    try {
        ...
        if (!psgcBrgy) {
            res.status(400).json({ message: 'Barangay not found in PSGC data' });
            return;
        }
...
export const getRegisteredProvinces = async (_req, res, next) => {
    try {
