...
const INCIDENT_TYPES = [
  ...
  'Property Damage', 'Missing Person'
];
...
async function seed() {
  ...
  // --- 2. Province: Antique ---
  const psgcProvince = psgc.provinces.find(p => p.code === '064500000');
  ...
  // --- 4. Barangays (3 per municipality = 15 total) ---
  const barangayDocs = {};
  ...
  // --- 7. Barangay accounts (one per barangay) ---
  const barangayUserDocs = {};
  for (const b of psgc.barangays) {
    const brgy = barangayDocs[b.code];
    const mun = municipalityDocs[b.municipalityCode];
    if (!brgy || !mun) continue;

    // Include municipality slug to avoid collisions (e.g. multiple "Brgy. Poblacion")
    ...
    if (!u) {
      u = await User.create({
        username: slug,
        password: 'Password@2025',
        fullName: `Barangay Admin - ${b.name}, ${mun.name}`,
        role: 'barangay',
        barangay: brgy._id,
        municipality: mun._id,
        province: province._id,
        scopeLabel: `${b.name}, ${mun.name}, Antique`,
        isActive: true
      });
  ...
  if (existingBlotters === 0) {
    ...
    for (let i = 0; i < sampleTopics.length; i++) {
      ...
      await Blotter.create({
        blotterNumber: `BLT-${mun.psgcCode}-${String(i + 1).padStart(4, '0')}`,
        barangay: brgy._id,
        municipality: mun._id,
        province: province._id,
        createdBy: brgyUser._id,
        status,
        ...
        incident: {
          ...
          isReferred: status === 'referred_to_pnp'
        },
        complainant: {
          lastName: t.complaint,
          firstName: 'dela Cruz',
  ...
  console.log('  Barangay:    brgy_ilaya_bugasong / Password@2025');
