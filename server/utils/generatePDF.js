...
export function generateBlotterPDF(blotter, res) {
  ...
  doc.text(`Province of ${prov?.name || 'Antique'}`, { align: 'center' });
  doc.text(`Municipality of ${mun?.name || ''}`, { align: 'center' });
  doc.text(`${brgy?.name || 'Barangay'}`, { align: 'center' });
  doc.moveDown(0.5);
  ...
  doc.font('Helvetica').text(`${c?.sex || 'N/A'}   Age: ${c?.age || 'N/A'}   Civil Status: ${c?.civilStatus || 'N/A'}`);
  doc.font('Helvetica-Bold').text('Address: ', { continued: true });
  doc.font('Helvetica').text(formatAddress(c?.address));
  doc.font('Helvetica-Bold').text('Contact: ', { continued: true });
  doc.font('Helvetica').text(c?.contactNumber || 'N/A');
  doc.font('Helvetica-Bold').text('Occupation: ', { continued: true });
  doc.font('Helvetica').text(c?.occupation || 'N/A');
  if (c?.idType) {
    doc.font('Helvetica-Bold').text('ID: ', { continued: true });
    doc.font('Helvetica').text(`${c.idType} - ${c.idNumber || ''}`);
  }
  doc.moveDown(0.5);
  ...
  respondents.forEach((r, idx) => {
    ...
    doc.font('Helvetica').text(`Name: ${formatName(r)}`);
    doc.text(`Sex: ${r.sex || 'N/A'}   Age: ${r.age || 'N/A'}`);
    doc.text(`Address: ${formatAddress(r.address)}`);
    doc.text(`Contact: ${r.contactNumber || 'N/A'}`);
    doc.text(`Relationship: ${r.relationshipToComplainant || 'N/A'}`);
    doc.moveDown(0.3);
  });
  ...
  if (witnesses.length > 0) {
    ...
    witnesses.forEach((w, idx) => {
      ...
      doc.font('Helvetica').text(`Name: ${w.firstName || ''} ${w.lastName || ''}`);
      doc.text(`Contact: ${w.contactNumber || 'N/A'}`);
      doc.text(`Address: ${w.address || 'N/A'}`);
      if (w.statement) doc.text(`Statement: ${w.statement}`);
      doc.moveDown(0.3);
    });
  }
  ...
  doc.font('Helvetica-Bold').text(`${brgy?.punongBarangay || 'Punong Barangay'}`, 320, doc.y - doc.currentLineHeight());
...
export function generateSummaryPDF(blotters, scope, dateRange, res) {
  const doc = new PDFDocument({ margin: 50, size: 'LETTER' });
  res.setHeader('Content-Type', 'application/pdf');
  ...
  blotters.forEach(b => {
    ...
    typeCounts[b.incident?.type] = (typeCounts[b.incident?.type] || 0) + 1;
  });
  ...
  blotters.forEach((b) => {
    ...
    const cols = [
      ...
      b.incident?.type || 'N/A',
      `${b.complainant?.lastName || ''}, ${b.complainant?.firstName || ''}`,
